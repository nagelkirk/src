...
# Are we doing the files witht he shoulder image included?
do.shoulder.image <- T
...
if(do.shoulder.image){
  image_folder <- "./Landsat/20km_Buffer/With_shoulder"
}else{
  image_folder <- "./Landsat/20km_Buffer"
}
...
for(k in 1:length(park_names)){
  ...
  for(p in 1:length(unique.paths)){
    ...
    # If doing the shoulder, just merge that one for now
    if(do.shoulder.image){
      path.images <- path.images[grepl("tran", path.images)]
      path_prefixes <- path_prefixes[grepl("tran", path_prefixes)]
    }
    ...
    for(i in 1:length(path_prefixes)){
      ...
      if(file.exists(paste0(substr(final.tif.name, 1, nchar(final.tif.name)-25), "minmax_NDVI_wo_indices_allBands.tif"))){
      ...
      # Check for the transition variation, too 
      
      # If the merged hdr and tif shoulder files exist, delete the unmerged files and go to next pair in loop
      ...
      for(j in 1:length(images_to_merge)){
        ...
        # num_of_bands <- dim(brick1)[3]
        # brick1_3off <- brick1[[-c((num_of_bands-4) : (num_of_bands-2))]]
        # #plot(brick1_3off[[9]])
        
        #########  CHANGE ZEROS, HIGH and LOW VALUES to NA  ################################################################################
        
        # Could do this after merging to ensure all zeros are set to NA (which isn't happening in Selous)
        
        # For every band, set invalid values to NA
        #t <- 1
        for(t in 1:dim(brick1)[3]){
          # Set 0 to NA
          NAvalue(brick1[[t]]) <- 0
          ...
          if(t <= band_breaker){ 
            ...
            values(brick1[[t]])[values(brick1[[t]]) > 10000] <- NA # Set high and low values to NA
            values(brick1[[t]])[values(brick1[[t]]) <= 0] <- NA # Set anything below zero to NA. This wasn't done for Kruger and Mpala, but those points will be outliers in the TGS maps, and eliminated there. 
            #brick1_3off[[t]][brick1_3off[[t]] > 10000 | brick1_3off[[t]] < 0] <- NA
          }else{
            # The last two bands are NDVI and MSAVI2, which need to be adjusted to a diff range of values than the landsat bands 
            # The indices were multiplied by 1000 in GEE, so the range for NDVI would be 1000 to -1000. The same should go for MSAVI2 according to
            # The experiment:
            # NIR <- sample(0:10000, 1000)/10000
            # Red <- sample(0:10000, 1000)/10000
            # MSAVI2 = (2 * NIR + 1 - (((2 * NIR + 1) ^ 2)  - 8 * (NIR - Red))^.5) / 2  
            # summary(MSAVI2)
            # Change the values outside the range to NA
            values(brick1[[t]])[values(brick1[[t]]) > 1000] <- 1000
            values(brick1[[t]])[values(brick1[[t]]) < -1000] <- -1000 # Could also increase index values to 10,000 scale my *10 here
            #brick1_3off[[t]][brick1_3off[[t]] > 1000 | brick1_3off[[t]] < -1000] <- NA
          }
        }
      ...
      # Write the merged rasters
      if(do.shoulder.image){
        # For now, just write the tif because it's all I need for the figures I want to make (we aren't using the shoulder image in ENVI)
        ...
        if(i == 1){
          minmax_brick <- brick_merged
          rednirswir_brick <- brick_merged[[c(red, nir, swir)]]
        }else if(i > 1){
          # Stack them
          minmax_brick <- stack(c(minmax_brick, brick_merged))
    ...
    # Write the tifs
    if(do.shoulder.image){
      # For now, we won't print anything. If the completed stacks have to be used in ENVI in the future, can put the modified version of
      # the below lines in here
    }else{
      writeRaster(rednirswir_brick, paste0(substr(final.tif.name, 1, nchar(final.tif.name) - 25), "minmax_NDVI_red_nir_swir1"), format = "ENVI", overwrite = TRUE, datatype = 'INT2S') 
      writeRaster(minmax_brick, paste0(substr(final.tif.name, 1, nchar(final.tif.name)-25), "minmax_NDVI_wo_indices_allBands"), format = "ENVI", overwrite = TRUE, datatype = 'INT2S')
      writeRaster(minmax_brick, paste0(substr(final.tif.name, 1, nchar(final.tif.name)-25), "minmax_NDVI_wo_indices_allBands"), format = "GTiff", overwrite = TRUE, datatype = 'INT2S')
    }
...
# Below shouldn't be necessary for the shoulder image, so it is skipped 
  ...
  for(k in 1:length(park_names)){
    ...
    for(p in 1:length(unique.paths)){
      ...
      for(i in 1:length(path.images)){
        ...
        one.date.time <- tick()
        # Get the images for that date
        images_to_merge <- fls2[grepl(path.images[i], fls2)]
        file_prefix <- path.images[i]
        
        # Get the satellite number
        sat_num <- substr(file_prefix, nchar(file_prefix)-48, nchar(file_prefix)-48)
        ...
        if(file.exists(paste0(substr(file_prefix, 1, nchar(file_prefix)-25), "minmax_NDVI_wo_indices_allBands", ".tif"))){
          print(paste0("Already created ", substr(file_prefix, 1, nchar(file_prefix)-25), "minmax_NDVI_wo_indices_allBands", ".tif"))
          next
        }
        ...
        if(i == 1){
          ...
          # Stack them
          minmax_brick <- stack(c(minmax_brick, brick2))
          # Export
          # Write the minmax allband tif and the two hdrs 
          writeRaster(minmax_brick[[c(red[1], nir[1], swir[1], red[2], nir[2], swir[2])]], paste0(substr(file_prefix, 1, nchar(file_prefix) - 25), "minmax_NDVI_red_nir_swir1"), format = "ENVI", overwrite = TRUE, datatype = 'INT2S') 
          writeRaster(minmax_brick, paste0(substr(file_prefix, 1, nchar(file_prefix)-25), "minmax_NDVI_wo_indices_allBands"), format = "ENVI", overwrite = TRUE, datatype = 'INT2S')
          writeRaster(minmax_brick, paste0(substr(file_prefix, 1, nchar(file_prefix)-25), "minmax_NDVI_wo_indices_allBands"), format = "GTiff", overwrite = TRUE, datatype = 'INT2S')
          gc() # Garbage collection. This is unlikely to help, because the first file (the "min" image) is hanging up the "max" images a lot, and this just collects garbage after the max is ran
        }
