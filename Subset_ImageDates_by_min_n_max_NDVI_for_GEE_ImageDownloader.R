...
for(i in 1:nrow(ele_table_w_no_extraYears)){
  ...
  for(j in 1:length(paths)){
    ...
    max.img.index <- path_df[which(path_df$NDVI_mean == maxNDVI), "Satellite"]
    ...
    if(j == 1){
      ...
      dates_table$max_Satellite[i] <- max.img.index
    }else{
      ...
      new_row$max_Satellite <- max.img.index
...
names(LS_dates)
LS_dates_noReps <- unique(LS_dates[c("GEE_park", "WRS_PATH", "DATE_ACQUIRED", "Satellite", "max_DATE_ACQUIRED", "max_Satellite")]) # Any more columns than this will start causing replicates (e.g., "Year")
# 
# # Now put the dates together into one column
# for(k in 1:NROW(LS_dates_noReps)){
#   LS_dates_noReps$minmaxDates[k] <- list(c(LS_dates_noReps$DATE_ACQUIRED[k], LS_dates_noReps$max_DATE_ACQUIRED[k]))
# }

# Export the new table to be loaded in to fusion tables and imported to GEE
write.csv(LS_dates_noReps, paste0("./LandsatDownload_inputs/Image_Date_tables/Landsat_image_4year_span_dates_singleParks_minmaxNDVI_", summary_stat_boundary, "_n_parkCloudCover.csv"), row.names = F)
