...
save.class.pixel <- TRUE  # Should the classified image be saved to the image folder? Only save images once you have the classification you want. (ref: "Combine Covers")
...
if(file.exists(paste0("./section_", park.section, "/", ground.truth.image.folder)) == F){
  dir.create(paste0("./section_", park.section, "/", ground.truth.image.folder), recursive = T)
}

if(file.exists(paste0("./section_", park.section, "/", hand.truth.image.folder)) == F){
