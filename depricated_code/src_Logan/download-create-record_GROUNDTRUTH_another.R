...
if(ryan){
  ...
  image.boundaries <- "Kruger_Boundary_GE_Buffers_excluded" # this is the boundary file that also has the boundaries of the google imagery
  landsat.image <- "Kruger_backdrop_mosaic.tif"  # this needs to be a landsat image that covers the entire sample area. It serves as a template for point creation
  working.dir <- "E:/Dropbox/Shared/Logan/EleTree/data/ParkData/" # set this to the folder path that has all the park folders in it. Make sure it has "/" at the end.
  hand.truth.image.folder <- "manual_ground_truth_images" # this should stay the same. It is the folder within the park folder that will hold the hand-classified images
  ground.truth.image.folder <- "automated_ground_truth_images"  # this should also stay the same. Like above, except all sample images are saved here, not just the hand-classified
...
############################  Create DFs or Load Already Existing Ones ###########################################
# DF for manual classification entries
# If a file was already saved, this will read it in 
# Else, a new df is made. 
# That means this can be ran each time

if(length(list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand")) >= 1){
  num.saved.files <- length(list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand"))
  saved.file.name <- list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand")[num.saved.files]
  hand.truth.df <- read.csv(paste0("./section_", park.section, "/", saved.file.name))
...
if(length(list.files(paste0("./section_", park.section, "/manual_ground_truth_images"), pattern = "rgb")) >= 1){
  rgb.9090.names <- list.files(paste0("./section_", park.section, "/manual_ground_truth_images"), pattern = "9090_rgb", full.names = T)
  hand.images <- gsub("_9090_rgb", "", rgb.9090.names)
  sample.pxl.num <- gsub(".*sample", "", hand.images)
  sample.pxl.num <- as.numeric(gsub("\\_.*", "", sample.pxl.num))
  park.pxl.num <- gsub(".*pixel_park", "", hand.images)
  park.pxl.num <- as.numeric(gsub("\\_.*", "", park.pxl.num))
  cellnums.df <- as.data.frame(cbind(sample.pxl.num, park.pxl.num))
  names(cellnums.df) <- c("sample_cell", "park_cell")
} else {
  # Else create the variables from the values that were created earlier
...
# If the hand truth df already has been created and save, load that
if(length(list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand")) >= 1){
  num.saved.files <- length(list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand"))
  saved.file.name <- list.files(paste0("./section_", park.section), pattern = "groundtruth_doneByHand")[num.saved.files]
  hand.truth.df <- read.csv(paste0("./section_", park.section, "/", saved.file.name))
...
image.num <- 25 # Manually set this as you go. It represents the point you are classifying. So will be in range of 1-50.
...
# TREES 
tree.bright.thresh <- 1.15 # Change this setting until you like the tree cover 
...
if(record.class.values){
  ...
  if(cannot_classify){   
    hand.truth.df$cannot.classify[image.num] <- 1
  }else{hand.truth.df$cannot.classify[image.num] <- 0}
